...
bool IOWriter::IO_Write(const std::string& content) const 
{
    ...
    std::ofstream file(m_sFileName, std::ios::out | (m_bBinary ? std::ios::binary : NULL));
    if (!file.is_open()) {
        return false;
    }
...
bool IOWriter::IO_Append(const std::string& content) const 
{
    ...
    std::ofstream file(m_sFileName, std::ios::app | (m_bBinary ? std::ios::binary : NULL));
    if (!file.is_open()) {
        return false;
    }
    IO_WriteStream(file, content);
...
std::optional<std::string> IOReader::IO_Read(/*size_t num_bytes*/) const {
    ...
    std::ifstream file(m_sFileName, std::ios::in | (m_bBinary ? std::ios::binary : NULL));
    if (!file.is_open()) {
        return {};
    }
