...
CLinterFunction* CFunctionManager::DeclareFunction(const VarjusString& var, std::size_t index) {
	assert(!var.empty());
...
CLinterFunction* CFunctionManager::GetFunction(const VarjusString& var) {
	return ContainsFunction(var) ? m_oFunctions[var].get() : nullptr;
}
bool CFunctionManager::ContainsFunction(const VarjusString& name) const {
