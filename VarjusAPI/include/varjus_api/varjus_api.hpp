...
namespace Varjus
{
    ...
    struct State
    {
        ...
        VARJUS_API __VARJUS_ND Varjus::Success UseStdLibrary(const std::span<VarjusString>& ignore = {});
        ...
        VARJUS_API __VARJUS_ND Varjus::Success LoadScriptFromFile(const VarjusString& fullFilePath, Varjus::EncodingType characterSet=e_utf8);

        //This function expects a script
        //It should be noted that modules cannot be used as there is no working directory
        VARJUS_API __VARJUS_ND Varjus::Success LoadScript(const VarjusString& script);
        ...
        // addArgs: a callback to a function where the user can add their own fn main arguments
        VARJUS_API __VARJUS_ND IValue* ExecuteScript(void(*addArgs)(Varjus::CProgramRuntime* const ctx, IValues& receiver) = 0);
        ...
        VARJUS_API __VARJUS_ND Varjus::Success Abort();

        //The script is no longer executing
        VARJUS_API __VARJUS_ND Varjus::Success HasFinished();
        ...
        VARJUS_API __VARJUS_ND Varjus::Success AddNewStaticObject(const VarjusString& name, const OptionalCtor<void>& constructor);

        //Declare a C++ function that can be called from the script
        VARJUS_API __VARJUS_ND Varjus::Success AddNewCallback(const VarjusString& name, const Function_t& callback, std::size_t numArgs);
    ...
    private: // All of this is managed by the class, so there should never be a need to publicly access these :)
        ...
        Varjus::Success m_bScriptLoaded{ failure };
