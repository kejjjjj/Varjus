...
namespace Varjus {
    ...
    namespace fmt {
        ...
        [[nodiscard]] inline VarjusString format(const VarjusString& _fmt, Args&&... args) {
            ...
            std::vector<VarjusString> values = { Varjus::fmt::to_string(args)... };
            size_t argIndex = 0;
            VarjusString result;
...
#ifdef UNICODE
            out << LocaleConverter::u16string_to_ansi(Varjus::fmt::format(_fmt, std::forward<Args&&>(args)...));
#else
            out << Varjus::fmt::format(_fmt, std::forward<Args&&>(args)...);
#endif
